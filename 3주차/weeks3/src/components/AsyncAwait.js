...
const AsyncAwait = () => {
    ...
    */
    ...
    // // await 키워드가 사용된 비동기 함수 doAsync() 함수, return 시킨 일반함수 doAsync 두 종류의 함수에서 발생하는 예외 모두 catch 문에서 처리됨
    // // getData가 async이고 await doAsync()함수가 await 함수가 아니었다면 doAsync 함수의 예외는 catch 문에서 처리되지 않는다. doAsync 함수의 처리가 끝나는 시점을 알 수 없기떄문에. 왜? 비동기니깐 언제 해줄지 모름 ㅋㅋ 
    // async function getData(){
    //     try {
    //         await doAsync();
    //         return doAsync();
